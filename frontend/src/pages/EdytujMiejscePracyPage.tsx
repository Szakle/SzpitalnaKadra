...
export default function EdytujMiejscePracyPage() {
      ...
      api.get(`/api/MiejscePracy/${id}`)
        .then(res => {
          ...
          setFormData({
            ...
            zawodSpecjalnoscId: '',
            kodFunkcji: data.kodFunkcji || '',
            nazwaFunkcji: data.nazwaFunkcji || '',
            rodzajZatrudnienia: data.rodzajZatrudnienia || 'ZATRUDNIONY U MNIE',
            pracaOd: data.pracaOd ? data.pracaOd.split('T')[0] : '',
            pracaDo: data.pracaDo ? data.pracaDo.split('T')[0] : '',
            bezterminowo: data.bezterminowo ?? !data.pracaDo,
            typHarmonogramu: data.typHarmonogramu || 'średniogodzinowy'
          });

          // Pobierz dane osoby i jej zawody
          return Promise.all([
            api.get(`/api/Osoba/${data.osobaId}`),
            api.get(`/api/ZawodySpecjalnosci/osoba/${data.osobaId}`)
          ]);
        })
        .then(([osobaRes, zawodyRes]) => {
          setOsoba(osobaRes.data);
          setZawodyOsoby(zawodyRes.data);
          setLoading(false);
        })
        .catch(err => {
  ...
  const handleMiejsceChange = (miejsceId: string) => {
    const selectedMiejsce = miejscaSwiadczen.find(m => m.id === parseInt(miejsceId));
    setFormData({
      ...formData,
      miejsceSwiadczenId: miejsceId,
      kodMiejscaUdzielaniaSwiadczen: selectedMiejsce?.kod || '',
      nazwaMiejscaUdzielaniaSwiadczen: selectedMiejsce ? `${selectedMiejsce.nazwa}, ${selectedMiejsce.adres}` : ''
    });
  };

  const handleZawodChange = (zawodId: string) => {
    const wybranyZawod = zawodyOsoby.find(z => z.id === parseInt(zawodId));
    setFormData({
      ...formData,
      zawodSpecjalnoscId: zawodId,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.put(`/api/MiejscePracy/${id}`, {
        ...
        kodSpecjalnosci: formData.kodSpecjalnosci,
        nazwaSpecjalnosci: formData.nazwaSpecjalnosci,
        zawodSpecjalnosc: formData.zawodSpecjalnosc,
      ...
      showToast('Miejsce pracy zostało zaktualizowane', 'success');
      navigate(`/miejsca-pracy/${miejsce.osobaId}`);
    } catch (error) {
      console.error('Błąd podczas aktualizacji miejsca pracy:', error);
      showToast('Nie udało się zaktualizować miejsca pracy', 'error');
    }
  };
    ...
    <div style={{
      ...
      <form onSubmit={handleSubmit} style={{
        ...
        <div style={{ marginBottom: '1.5rem' }}>
          ...
          {formData.nazwaMiejscaUdzielaniaSwiadczen && !formData.miejsceSwiadczenId ? (
            <div>
              <input
                type="text"
                value={formData.nazwaMiejscaUdzielaniaSwiadczen}
                readOnly
                style={{ ...inputStyle, backgroundColor: '#f5f5f5', marginBottom: '0.5rem' }}
              />
              <select
                ...
                style={inputStyle}
              >
                <option value="">-- zmień na inne miejsce --</option>
        ...
        {/* Zawód/specjalność - wybór z listy zawodów osoby lub pokazanie aktualnego */}
        <div style={{ marginBottom: '1.5rem' }}>
          <label style={labelStyle}>Zawód/specjalność:*</label>
          {formData.zawodSpecjalnosc && !formData.zawodSpecjalnoscId ? (
            <div>
              <input
                type="text"
              ...
              {zawodyOsoby.length > 0 && (
                <select
                  ...
                  style={inputStyle}
                >
                  <option value="">-- zmień na inny zawód --</option>
                  {zawodyOsoby.map(z => (
            ...
            <select
              ...
              onChange={(e) => handleZawodChange(e.target.value)}
              style={inputStyle}
              required={!formData.zawodSpecjalnosc}
            >
              <option value="">-- wybierz --</option>
              {zawodyOsoby.map(z => (
                <option key={z.id} value={z.id}>
